...
class Register:
    def __init__(self,root):
        ...
        self.root.geometry("1320x640+0+0")
        ...
        btn_login = Button(self.root,text="Sign in",command= self.login_window,font=("times new roman",20),bd=0,cursor="hand2").place(x=200,y=480,width=160)
    ...
    def register_data(self):
        if self.txt_fname.get()=="" or self.txt_email.get()=="" or self.txt_password.get()=="" or self.txt_cpassword.get()=="":
            ...
            try:
                ...
                row= cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","User already exist, try with another mail",parent=self.root)
                else:
