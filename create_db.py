...
def create_db():
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS student(roll INTEGER PRIMARY KEY AUTOINCREMENT,name text,email text,gender text,dob text,contact text,admission test,course ,state text,city text, pin text,address text)")
    con.commit()
